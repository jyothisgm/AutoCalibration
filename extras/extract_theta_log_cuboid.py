...
RE_HEADER = re.compile(r"#\s*cuboid=(\S+)\s+scenario=(\S+)\s+N_ANGLES=(\d+),\s*K=(\d+)")
RE_VALUES = re.compile(r"^(?!#)([0-9eE+.\-]+(?:\s+[0-9eE+.\-]+)+)\s*$", re.MULTILINE)
RE_SUM    = re.compile(r"#\s*sum\s*=\s*([0-9eE+.\-]+)")

THETA_LOG_DIR = Path(__file__).resolve().parent.parent / "simulated" / "theta_log_cuboid"
...
def parse_theta_file(path: Path) -> list[dict]:
    ...
    for block in blocks:
        ...
        cuboid   = header_m.group(1)
        scenario = header_m.group(2)
        n_angles = int(header_m.group(3))
        k        = int(header_m.group(4))
...
def parse_all(folder: Path = THETA_LOG_DIR) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df = (df.sort_values(["cuboid", "K", "N", "_scen_num"])
                .drop(columns="_scen_num")
                .reset_index(drop=True))

        priority = ["cuboid", "K", "N", "scenario"]
...
EXPECTED_CUBOIDS = ["compact", "small", "square", "tall", "wide"]
EXPECTED_K       = [3]
...
def find_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected = pd.DataFrame(
        list(itertools.product(EXPECTED_CUBOIDS, EXPECTED_K, EXPECTED_N, EXPECTED_SC)),
        columns=["cuboid", "K", "N", "scenario"],
    )
    found = df[["cuboid", "K", "N", "scenario"]].drop_duplicates()
    missing = expected.merge(found, on=["cuboid", "K", "N", "scenario"], how="left", indicator=True)
    missing = missing[missing["_merge"] == "left_only"].drop(columns="_merge")
    missing = (missing.sort_values(["cuboid", "N", "K", "scenario"])
                      .reset_index(drop=True)[["cuboid", "N", "K", "scenario"]])
    return missing
...
def plot_heatmap(df: pd.DataFrame, out_path: Path) -> None:
    ...
    cuboids = [c for c in EXPECTED_CUBOIDS if c in df["cuboid"].unique()]
    ...
    # Single heatmap: rows = cuboid sizes, columns = N
    pivot = (
        df.assign(abs_sum=df["sum"].abs())
          .groupby(["cuboid", "N"])["abs_sum"]
          .mean()
          .unstack("N")
          .reindex(index=cuboids, columns=EXPECTED_N)
    )

    bracket_grid = pivot.map(to_bracket).values.astype(float)

    fig, ax = plt.subplots(figsize=(14, max(3, len(cuboids) * 0.7 + 1.5)))
    ...
    ax.set_yticklabels(cuboids, fontsize=9)
    ax.set_xlabel("N (number of angles)", fontsize=10)
    ax.set_ylabel("Cuboid size", fontsize=10)

    for ri, cuboid in enumerate(cuboids):
        for ci, n in enumerate(EXPECTED_N):
            val = pivot.loc[cuboid, n] if (cuboid in pivot.index and n in pivot.columns) else float("nan")
            if not pd.isna(val):
    ...
    ax.set_title("Mean |sum| by cuboid size vs N  (K=3, averaged over scenarios)", fontsize=11)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    if not df.empty:
        print(df[["cuboid", "scenario", "N", "K", "sum"]].to_string(index=False))

    out = THETA_LOG_DIR.parent / "theta_log_cuboid_summary.csv"
    df.to_csv(out, index=False)
    print(f"\nSaved: {out}")

    # --- Missing combinations ---
    missing = find_missing(df)
    total_expected = len(EXPECTED_CUBOIDS) * len(EXPECTED_K) * len(EXPECTED_N) * len(EXPECTED_SC)
    print(f"\n{'='*50}")
    ...
    if missing.empty:
        ...
        out_missing = THETA_LOG_DIR.parent / "theta_log_cuboid_missing.csv"
        missing.to_csv(out_missing, index=False)
        print(f"\nSaved missing list: {out_missing}")

    # --- Heatmap ---
    if not df.empty:
        out_heatmap = THETA_LOG_DIR.parent / "theta_log_cuboid_heatmap.png"
