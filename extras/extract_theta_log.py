...
RE_HEADER = re.compile(r"#\s*scenario=(\S+)\s+N_ANGLES=(\d+),\s*K=(\d+)")
RE_VALUES = re.compile(r"^(?!#)([0-9eE+.\-]+(?:\s+[0-9eE+.\-]+)+)\s*$", re.MULTILINE)
RE_SUM    = re.compile(r"#\s*sum\s*=\s*([0-9eE+.\-]+)")

THETA_LOG_DIR = Path(__file__).resolve().parent.parent / "simulated" / "theta_log"
...
def parse_theta_file(path: Path) -> list[dict]:
    ...
    for block in blocks:
        ...
        scenario = header_m.group(1)
        n_angles = int(header_m.group(2))
        k        = int(header_m.group(3))
...
def parse_all(folder: Path = THETA_LOG_DIR) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df = df.sort_values(["K", "N", "_scen_num"]).drop(columns="_scen_num").reset_index(drop=True)

        # Reorder columns: N, K, scenario first
        priority = ["K", "N", "scenario"]
...
def find_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    expected = pd.DataFrame(
        list(itertools.product(EXPECTED_K, EXPECTED_N, EXPECTED_SC)),
        columns=["K", "N", "scenario"],
    )
    found = df[["K", "N", "scenario"]].drop_duplicates()
    missing = expected.merge(found, on=["K", "N", "scenario"], how="left", indicator=True)
    missing = missing[missing["_merge"] == "left_only"].drop(columns="_merge")
    missing = missing.sort_values(["N", "K", "scenario"]).reset_index(drop=True)[["N", "K", "scenario"]]
    return missing


BRACKETS      = [0, 0.1, 1, 3, 10, float("inf")]
BRACKET_LABELS = ["0–0.1", "0.1–1", "1–3", "3–10", "10+"]
BRACKET_COLORS = ["#1a9641", "#a6d96a", "#ffffbf", "#fdae61", "#d7191c"]  # green → red
...
def plot_heatmap(df: pd.DataFrame, out_path: Path) -> None:
    ...
    pivot = (
        df.assign(abs_sum=df["sum"].abs())
          ...
          .mean()
          .unstack("N")
          .reindex(index=list(reversed(EXPECTED_K)), columns=EXPECTED_N)
    )

    # Map each value to a bracket index (0-4), NaN stays NaN
    def to_bracket(val):
        if pd.isna(val):
            return float("nan")
    ...
    norm = mcolors.BoundaryNorm(boundaries=[-0.5 + i for i in range(len(BRACKET_COLORS) + 1)],
                                ncolors=len(BRACKET_COLORS))

    fig, ax = plt.subplots(figsize=(14, 5))
    im = ax.imshow(bracket_grid, aspect="auto", cmap=cmap, norm=norm)

    ax.set_xticks(range(len(EXPECTED_N)))
    ax.set_xticklabels(EXPECTED_N, rotation=45, ha="right", fontsize=8)
    ax.set_yticks(range(len(EXPECTED_K)))
    ax.set_yticklabels([f"K={k}" for k in reversed(EXPECTED_K)])

    # Annotate each cell with the actual value
    for ri, k in enumerate(reversed(EXPECTED_K)):
        for ci, n in enumerate(EXPECTED_N):
            val = pivot.loc[k, n] if (k in pivot.index and n in pivot.columns) else float("nan")
            if not pd.isna(val):
                b_idx = int(to_bracket(val))
                # Use white text on dark cells (first and last bracket)
    ...
    ax.legend(handles=patches, title="mean |sum|", bbox_to_anchor=(1.13, 1),
              loc="upper left", fontsize=8, title_fontsize=8, framealpha=0.9)

    ax.set_xlabel("N (number of angles)", fontsize=10)
    ax.set_ylabel("K", fontsize=10)
    ax.set_title("Mean |sum| per scenario  ·  by N and K", fontsize=11)
...
if __name__ == "__main__":
    df = parse_all()
    print(f"Parsed {len(df)} rows from {df['file'].nunique()} files")
    print(df[["file", "scenario", "N", "K", "sum"]].to_string(index=False))

    out = THETA_LOG_DIR.parent / "theta_log_summary.csv"
    df.to_csv(out, index=False)
    print(f"\nSaved: {out}")

    # --- Missing combinations ---
    missing = find_missing(df)
    total_expected = len(EXPECTED_K) * len(EXPECTED_N) * len(EXPECTED_SC)
    print(f"\n{'='*50}")
    ...
    if missing.empty:
        ...
        out_missing = THETA_LOG_DIR.parent / "theta_log_missing.csv"
        missing.to_csv(out_missing, index=False)
        print(f"\nSaved missing list: {out_missing}")

    # --- Heatmap ---
    out_heatmap = THETA_LOG_DIR.parent / "theta_log_heatmap.png"
