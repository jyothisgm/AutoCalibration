...
INITIAL_CONFIG_PATH = HERE.parent / "Assets" / "StreamingAssets" / "initial_config.json"
...
def build_geometry_from_scan_dir(scan_dir: Path, obj_y, out_line_integrals_subdir="out_line_integrals"):
    settings_path = Path(scan_dir) / "scan settings.txt"
    ...
    proj_dir = Path(scan_dir) / out_line_integrals_subdir
    ...
    return {
        ...
        "obj":               np.array([p["tra_obj"],  obj_y,         p["mag_obj"]], dtype=np.float32),
...
def extract_scan_info(scan_root: Path, obj_y):
    scan_root = Path(scan_root)
    if not (scan_root / "scan settings.txt").exists():
        raise FileNotFoundError(f"No 'scan settings.txt' found in {scan_root}")

    info = build_geometry_from_scan_dir(scan_root, obj_y)

    print(f"\n{'='*55}")
    print(f"Scan info: {scan_root / 'scan settings.txt'}")
    print(f"{'='*55}")
    print(f"  src (tube)  : x={info['src'][0]:10.6f}, y={info['src'][1]:10.6f}, z={info['src'][2]:10.6f}")
    print(f"  obj         : x={info['obj'][0]:10.6f}, y={info['obj'][1]:10.6f}, z={info['obj'][2]:10.6f}")
    print(f"  det         : x={info['det'][0]:10.6f}, y={info['det'][1]:10.6f}, z={info['det'][2]:10.6f}")
    print(f"  image_width : {info['image_width']}")
    print(f"  image_height: {info['image_height']}")
    print(f"  det_spacing : {info['det_spacing']:.6f} mm  (binned pixel size)")
    if info["SOD"] is not None:
        print(f"  SOD         : {info['SOD']:.6f} mm")
    if info["SDD"] is not None:
        print(f"  SDD         : {info['SDD']:.6f} mm")
    if info["initial_angle_deg"] is not None:
        print(f"  initial_angle: {info['initial_angle_deg']:.6f} deg")
    print(f"  projections : {info['projections']}")
    print(f"{'='*55}\n")
...
def build_residual_image_based(theta, real_df, angles_deg, cfg, pred_dir, debug=True):
    ...
    fetch_and_save_projections(
        ...
        phantom_name=HERE / cfg["phantom_name"],
        debug=debug,
    )
...
def save_calibration_to_config(theta, initial_calibration, mount_height, config_path=INITIAL_CONFIG_PATH):
    dSx, dSy, dOx, dOy, dOz, dDx, dDy, dDz, alpha, offset_x, offset_z = theta
    ic = np.asarray(initial_calibration, dtype=np.float64)
    src_off = ic[0] + np.array([dSx, dSy, 0.0])
    obj_off = ic[1] + np.array([dOx, dOy, dOz])
    det_off = ic[2] + np.array([dDx, dDy, dDz])
    object_base_y = float(mount_height + dOy)
    config = {
        "sourceOffset":   {"x": float(src_off[0]), "y": float(src_off[1]), "z": float(src_off[2])},
        "objectOffset":   {"x": float(obj_off[0]), "y": float(0.0),        "z": float(obj_off[2])},
        "detectorOffset": {"x": float(det_off[0]), "y": float(det_off[1]), "z": float(det_off[2])},
        "alpha":          float(alpha),
        "offsetX":        float(offset_x),
        "offsetZ":        float(offset_z),
        "objectBaseY":    object_base_y,
    }
...
# MAIN
# -----------------------------------------------------------------------
def main(scan_root, angles, K, phantom_name="phantom.npy", mount_height=0.0):
    scan_root     = Path(scan_root)
    ...
    phantom = np.load(HERE / phantom_name)
    phantom_height_mm = phantom.shape[0] * VOXEL_SIZE
    
    obj_y = mount_height + phantom_height_mm / 2.0

    # ---- Read all geometry from scan settings.txt ----
    info = extract_scan_info(scan_root, obj_y)
    ...
    save_calibration_to_config(theta_hat, initial_calibration, mount_height)
    return theta_hat
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--phantom",
        dest="phantom_name", type=str, default="phantom.npy",
        help="Phantom filename inside the python_server directory (default: phantom.npy).",
    )
    parser.add_argument(
        "-y", "--mount-height",
        dest="mount_height", type=float, default=0.0,
        help=f"Estimated mount height in Unity world units (default: {0.0}).",
    )
    args = parser.parse_args()

    main(scan_root=args.scan_root, angles=args.angles, K=args.K,
         phantom_name=args.phantom_name, mount_height=args.mount_height)
