...
class AstraServer:
    def __init__(self, object: np.ndarray, image_width=956, image_height=760, voxel_size=1.0):
...
def serve(host="127.0.0.1", port=50007, voxel_size=0.1):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            astra_server = None
            current_name, current_w, current_h = None, None, None

            try:
                while True:
                    # Per-frame: width(4) + height(4) + name_len(4) + name + geom(48)
                    header = _recv_exact(conn, 12)
                    if not header:
                        print("Client disconnected")
                        break
                    image_width, image_height, name_len = struct.unpack('<iii', header)
                    phantom_name = _recv_exact(conn, name_len).decode('utf-8')
                    geom_bytes = _recv_exact(conn, 12 * 4)
                    if not geom_bytes:
                        print("Client disconnected")
                        break

                    if phantom_name != current_name or image_width != current_w or image_height != current_h:
                        if astra_server:
                            astra_server.close()
                        print(f"Init: {image_width}x{image_height}, phantom={phantom_name}")
                        phantom_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), phantom_name)
                        rec = np.load(phantom_path)
                        astra_server = AstraServer(object=rec, image_width=image_width, image_height=image_height, voxel_size=voxel_size)
                        current_name, current_w, current_h = phantom_name, image_width, image_height

                    geometry_vector = np.frombuffer(geom_bytes, dtype=np.float32, count=12)
                    img = astra_server.generate_image(geometry_vector)
